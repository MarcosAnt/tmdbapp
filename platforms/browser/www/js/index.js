...
$$('#searchMovie').on('submit', function (event) {
  ...
  app.request.json(url, function (data) {
    ...
    $$("#searchField").attr('display', 'none');
    $$("#toogleSearchField").attr('display', 'block');
  });
});

$$('#toogleSearchField').on('click', function(){
  $$("#toogleSearchField").attr('display', 'none');
  $$("#searchField").attr('display', 'block');
});
